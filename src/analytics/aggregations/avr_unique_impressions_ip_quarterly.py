...
insert_query = '''
                ...
                SELECT 
                    master_business_id,
                    DATE_TRUNC('quarter',happened_at) AS date,
                ...
                (
                    ...
                    SELECT 
                        ...
                        (payload->>'ip_address') AS ip_address
                    FROM 
                        s3.avr_button_widget_was_rendered
                    WHERE 
                        happened_at >= %(start_date)s
                    AND 
                        happened_at < %(end_date)s
...
while True:
    ...
    try:
        ...
        min_date_avr_button_widget_was_rendered = postgreshandler.get_min_value(postgres_etl_connection, 's3','avr_button_widget_was_rendered','happened_at')
        ...
        max_date_avr_button_widget_was_rendered = postgreshandler.get_max_value(postgres_etl_connection, 's3','avr_button_widget_was_rendered', 'happened_at')

        max_date_avr_widget_impressions_ip_quarterly = postgreshandler.get_max_value(postgres_etl_connection, schema,script,'date')
        ...
        if min_date_avr_button_widget_was_rendered is None:
            raise Exception('min_date_avr_button_widget_was_rendered is None')

        first_date = utility.add_months(max_date_avr_widget_impressions_ip_quarterly,3) if max_date_avr_widget_impressions_ip_quarterly is not None else utility.get_quarter(min(min_date_integrations_widget_was_rendered,min_date_avr_widget_impressions,min_date_integrations_button_widget_was_rendered,min_date_avr_button_widget_was_rendered))
        last_date = utility.get_quarter(min(max_date_integrations_widget_was_rendered,max_date_avr_widget_impressions,max_date_integrations_button_widget_was_rendered,max_date_avr_button_widget_was_rendered))

        if first_date < last_date:
            for date in utility.get_quarters_from(first_date,last_date):
                start_date = date
                end_date = utility.add_months(start_date,3)
