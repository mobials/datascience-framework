...
query = '''
            ...
            WHERE 
                happened_at > COALESCE( 
                    (SELECT MAX(date) FROM avr_widget_impressions_ip_daily),
                    (SELECT DATE_TRUNC('day',MIN(happened_at)) FROM s3.avr_widget_impressions) + INTERVAL '1 day'
                    )
            AND 
                happened_at < (SELECT DATE_TRUNC('day',MAX(happened_at)) FROM s3.avr_widget_impressions)
            GROUP BY 
                1,2,3;
