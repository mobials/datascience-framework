...
query =     '''
                CREATE TEMP TABLE avr_client_new_listings_ca_used_temp ON COMMIT DROP AS 
                SELECT 
                    a.master_business_id, 
                    a.vin,
                    a.status_date
                FROM 
                    aggregations.avr_client_inventory_ca_used a
                WHERE 
                    NOT EXISTS 
                    (
                        SELECT 
                ...
                SELECT 
                    * 
                FROM 
                    avr_client_new_listings_ca_used_temp;
